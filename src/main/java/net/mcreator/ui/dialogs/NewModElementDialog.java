...
public class NewModElementDialog {
	...
	public static void showNameDialog(MCreator mcreator, ModElementType type) {
		String emptyRegName = L10N.t("dialog.new_mod_element.registry_name.empty");
		JLabel regName = L10N.label("dialog.new_mod_element.registry_name", emptyRegName);
		...
		String modName = VOptionPane.showInputDialog(mcreator, L10N.t("dialog.new_mod_element.message", type.getReadableName()),
				L10N.t("dialog.new_mod_element.title", type.getReadableName()), TiledImageCache.getModTypeIcon(type),
				new OptionPaneValidatior() {
					@Override public Validator.ValidationResult validate(JComponent component) {
						String elementName = ((VTextField) component).getText();
						regName.setText(L10N.t("dialog.new_mod_element.registry_name", elementName.equals("")
								? emptyRegName : RegistryNameFixer.fromCamelCase(elementName)));
						return new ModElementNameValidator(mcreator.getWorkspace(), (VTextField) component).validate();
					}
				}, L10N.t("dialog.new_mod_element.confirm", type.getReadableName()),
						L10N.t("dialog.new_mod_element.cancel"), null, regName);
